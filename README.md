...
已按两套数据准备好配置文件，把 `base.log` / `rove.log` 放进对应目录后直接运行：

| 数据集 | 基线类型 | 配置文件 | 数据目录 |
|--------|----------|----------|----------|
| 20240301 | 零基线（共用天线，真实基线为 0） | `config/config_20240301.ini` | `data/20240301/` |
| 20240303 | 短基线，长度 5.7264 m | `config/config_20240303.ini` | `data/20240303/` |
...
1. 零基线数据 `data/20240301/`，运行 `GNSS_RTK.exe config\config_20240301.ini`。
   真实基线为 0，`dX dY dZ` 应在毫米级，`Type` 列应很快稳定在 3（固定解），Ratio 很高。
2. 短基线数据 `data/20240303/`，运行 `GNSS_RTK.exe config\config_20240303.ini`，
