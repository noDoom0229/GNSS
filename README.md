...
- 文件模式：基准站与流动站各一个 NovAtel OEM 原始数据流文件（`base.log` / `rove.log`），
  放在 `data/` 并在配置中指定。扩展名 `.log` 还是 `.bin` 无所谓，程序按二进制打开。
- 文件需为 **OEM7 二进制**格式（同步头 `AA 44 12`），不能是 ASCII（`#RANGEA,...`）。
- 文件需包含 RANGE(43)、GPS 星历(7)、BDS 星历(1696)、BESTPOS(42)/PSRPOS(47) 消息。
  若数据录的是 RANGECMP(140) 压缩观测值，当前解码器不支持，需重新录 `LOG RANGEB`。

### 实测数据集（武汉大学测绘学院院楼楼顶，OEM729 板卡）

数据文件较大，不入库（已在 `.gitignore` 中忽略），需自行下载后放到 `data/` 目录：

```
data/
├── 20240301_base.log   零基线 基准站
├── 20240301_rove.log   零基线 流动站
├── 20240303_base.log   短基线 基准站
└── 20240303_rove.log   短基线 流动站
```

对应的配置文件已经写好，放好数据直接运行即可：

| 数据集 | 采集时间 | 基线类型 | 配置文件 |
|--------|----------|----------|----------|
| 20240301 | 2024-03-01 上午 | 零基线（共用天线，真实基线为 0） | `config/config_20240301.ini` |
| 20240303 | 2024-03-03 上午 | 短基线，长度 5.7264 m | `config/config_20240303.ini` |

两套数据的基准站坐标相同：`(-2267808.6227, 5009324.7699, 3221016.8928)` m (WGS84 ECEF)。
`RINEX/` 是由 `.log` 转出的 RINEX 格式，本程序不使用；`res/` 是参考结果，用于对照（见第 12 节）。

> `main` 分支里的 `NovatelOEM20211114-*.log` 是上学期 SPP 工程用的**单站**数据，
> 只能跑单点定位，不能用于 RTK——RTK 需要基准站与流动站两台接收机的同步观测。

建议先跑 20240301 零基线：真实基线为 0，`dX dY dZ` 与 `dE dN dU` 都应在毫米级，
双差相位残差就是接收机噪声本身，是检验整条解算链路最干净的一组数据。

> **关于 20240303 的基线方向**：数据说明给出 XYZ 基线 `(-1.398, 2.575, -4.920)` 与
> ENU 基线 `(0.211, -5.723, 0.018)`，两者长度一致（5.7264 m）但三个分量符号相反，
> 只有一个是「流动站 − 基准站」。`config_20240303.ini` 默认按 XYZ 基线取流动站参考坐标，
> 并在注释里给出了另一组取值。这只影响 `dE/dN/dU` 三列的参考值，不影响解算本身；
> 对照 `res/RTKRes.txt` 第一行即可确定用哪一组。
...
1. 零基线：运行 `GNSS_RTK.exe config\config_20240301.ini`。
   真实基线为 0，`dX dY dZ` 应在毫米级，`Type` 列应很快稳定在 3（固定解），Ratio 很高。
2. 短基线：运行 `GNSS_RTK.exe config\config_20240303.ini`，
   `dX dY dZ` 应接近 `(-1.398, 2.575, -4.920)` m（符号约定见第 6 节）。
3. 与参考结果对照：
   - `res/roveSPPRes.txt`、`res/baseSPPRes.txt` 对 SPP 链路
   - `res/RTKRes.txt` 对最终坐标与基线
   - `res/RTKInfo.txt` 对中间结果：参考星、共视卫星数、双差模糊度、双差相位/伪距残差、
     Ratio 与固定状态。逐历元对比可以定位到具体是哪个模块出偏差。
4. 检查 `output/*.nmea`，可载入 RTKPLOT 查看轨迹与 fix 状态。
