...
static void format_line(char* buf, int size, PosRes& rovPosRes, PosRes& basPosRes,
    DDCObs& ddObs, SDEpochObs& sdObs, const double refXYZ[3])
{
    double dXYZ[3];
...
{
    const char* header =
        "%WEEK        SOW              X              Y              Z         dX         dY         dZ    Ratio Sat Ty        dE        dN        dU";
    if (fp != NULL)
    {
        fprintf(fp, "%s\n", header);
        fflush(fp);
...
{
    char buf[512];
    format_line(buf, sizeof(buf), rovPosRes, basPosRes, ddObs, sdObs, refXYZ);
    printf("%s\n", buf);
}
